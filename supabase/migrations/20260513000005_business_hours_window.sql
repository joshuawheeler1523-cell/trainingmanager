-- Implementation-wide business-hours window.
--
-- Adds two new columns to public.implementations:
--   business_hours_start_local — earliest hour-of-day any session may start
--   business_hours_end_local   — latest hour-of-day any session may end
--
-- Defaults (0 .. 24) preserve the prior behavior: rooms keep their own
-- start_hour_local / available_hours_per_day. When the user tightens the
-- window — say "8 AM to 5 PM" — every room's effective day is clamped to
-- max(room.start, biz.start) .. min(room.end, biz.end), and the SQL
-- generator + the in-process simulator both honor it.
--
-- This is intentionally distinct from per-room hours: rooms keep their
-- preferred default ("our lab opens at 9"), and the impl gates the
-- organization-wide policy ("no class before 8, none ends after 5").
-- If clamping leaves a room with an empty window on a day, the generator
-- silently skips that room — same shape as the existing equipment filter.

alter table public.implementations
  add column if not exists business_hours_start_local numeric(4,2) not null default 0
    check (business_hours_start_local >= 0 and business_hours_start_local <= 24);

alter table public.implementations
  add column if not exists business_hours_end_local numeric(4,2) not null default 24
    check (business_hours_end_local >= 0 and business_hours_end_local <= 24);

alter table public.implementations
  drop constraint if exists implementations_business_hours_window_chk;
alter table public.implementations
  add constraint implementations_business_hours_window_chk
    check (business_hours_end_local >= business_hours_start_local);

-- ── Generator: respect the business-hours window ───────────────────────────
-- Forks 20260511000010_generator_respects_trainer_pto.sql with two changes
-- in the room loop:
--   1. Load v_biz_start_hr / v_biz_end_hr alongside the other impl scalars
--      so we can clamp every room's daily window without an extra query.
--   2. Replace the bare room.start_hour_local / room.start_hour_local +
--      available_hours_per_day computations with greatest(...) / least(...)
--      against the biz window, and skip rooms with empty effective windows.
-- Everything else is byte-for-byte the prior function.
    ...
    for v_session_idx in 1..v_sessions_needed loop
      ...
      while v_day <= v_cutoff_date loop
        ...
        loop
          ...
          -- Effective window = room hours clamped to impl business hours.
          -- If lunch falls inside the room's instruction span, extend the
          -- end by lunch length (rooms get their full instruction hours
          -- even when lunch happens mid-day) — then clamp THAT to biz.end.
          ...
          -- If the biz-hours clamp eliminates the room's day, skip.
