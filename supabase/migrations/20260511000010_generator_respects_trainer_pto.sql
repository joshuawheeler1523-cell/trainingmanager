-- Generator: respect trainer PTO/unavailability windows.
--
-- Replaces the Phase 4.7 lunch-span generator (20260511000008). All prior
-- logic preserved; one block added: alongside the in-impl published-session
-- and cross-impl pre-seeds, we also pre-seed tmp_busy_trainer from the
-- trainer's impl_trainer_unavailability rows. The greedy loop's existing
-- overlap check then naturally pushes placements past PTO windows.
...
begin
  ...
  -- PTO / unavailability pre-seed: each impl_trainer_unavailability row for
  -- a trainer in this implementation becomes a busy interval. The greedy
  -- loop's existing overlap check naturally skips slots that intersect.
  for rec_pto in
