-- Generator dry-run mode.
--
-- Adds a `p_dry_run boolean default false` parameter to
-- generate_implementation_schedule. In dry-run mode the function still does
-- all the planning work (greedy placement, conflict detection, gap math)
-- and returns the same JSON shape — but it does NOT delete existing draft
-- rows or insert new ones. This lets the Calculate page run the actual
-- production scheduler as a feasibility probe instead of running an
-- in-memory simulator that drifts from the SQL over time.
--
-- Signature change → DROP + CREATE (CREATE OR REPLACE can't add params).
-- Permissions re-granted at the bottom.
--
-- The function body is a fork of 20260513000005_business_hours_window.sql
-- with two changes:
--   1. `p_dry_run boolean default false` added to the signature.
--   2. The two write statements (DELETE draft + INSERT placement) are
--      wrapped in `if not p_dry_run then ... end if;`.
-- Everything else is byte-for-byte identical.

drop function if exists public.generate_implementation_schedule(uuid);
...
begin
  ...
  -- DRY-RUN: never delete existing drafts. The function still simulates
  -- placement using the temp tables (which start fresh per call) and the
  -- return JSON reflects what WOULD happen.
  ...
  loop
    ...
    for v_session_idx in 1..v_sessions_needed loop
      ...
      if v_slot_start is null then
        ...
        -- DRY-RUN: don't actually insert; just count what WOULD be placed.
        if not p_dry_run then
...
revoke execute on function public.generate_implementation_schedule(uuid, boolean) from public, anon;
grant execute on function public.generate_implementation_schedule(uuid, boolean) to authenticated;
