-- Phase C of the 2026-05-11 Training Planner overhaul:
-- Add business-hours per room, lunch break per implementation, and
-- structured equipment tags on both rooms and classes. Update the
-- generator to honor all three.
--
-- See docs/build-plans/2026-05-11_training-planner-calculate-overhaul.md.

-- ── Columns ────────────────────────────────────────────────────────────────

alter table public.impl_rooms
  add column if not exists start_hour_local numeric(4,2) not null default 9.0
    check (start_hour_local >= 0 and start_hour_local < 24);

alter table public.impl_rooms
  add column if not exists timezone text;
-- Nullable: falls back to the org's time_zone (organizations.time_zone), then
-- 'America/New_York' as a last resort. We don't enforce a tz allow-list here
-- because the UI controls input and Postgres will raise on a bad zone at
-- AT TIME ZONE evaluation.

alter table public.impl_rooms
  add column if not exists equipment_tags text[] not null default '{}';

alter table public.impl_classes
  add column if not exists required_equipment_tags text[] not null default '{}';

alter table public.implementations
  add column if not exists lunch_break_start_minutes int not null default 720
    check (lunch_break_start_minutes between 0 and 1439);
-- 720 = noon. Minutes from local midnight.

alter table public.implementations
  add column if not exists lunch_break_length_minutes int not null default 60
    check (lunch_break_length_minutes between 0 and 240);
-- 0 = no lunch break.

-- GIN index for equipment-tag subset queries (used by the generator's room
-- filter via `required_equipment_tags <@ equipment_tags`).
create index if not exists impl_rooms_equipment_tags_idx
  on public.impl_rooms using gin (equipment_tags);

-- ── Generator update ───────────────────────────────────────────────────────
-- Refines the Phase B generator (20260511000001_schedule_generator_v2.sql)
-- to use per-room start_hour_local + timezone, skip lunch interval, and
-- filter rooms by required_equipment_tags subset.
  ...
  loop
    ...
    for v_session_idx in 1..v_sessions_needed loop
      ...
      while v_day <= v_window_end loop
        -- Best-fit room order. Equipment filter: every required tag on the
        -- class must be present on the room (subset match). If the class has
        -- no required tags, all rooms pass.
        for rec_room in
          select r.*
        ...
        loop
          ...
          loop
            ...
            while v_local_hr + rec_class.hours_per_session <= v_day_end_local_hr loop
              -- Skip lunch: if the proposed slot overlaps the lunch window,
              -- push start to lunch_end and re-check fit.
              if v_lunch_active
