-- Fix: generate_implementation_schedule was inserting impl_sessions rows
-- without a department_id, which the 2026-05-08 multi-department migration
-- made NOT NULL. Generating a schedule failed with:
--   null value in column "department_id" of relation "impl_sessions" violates not-null constraint
--
-- Inherit department_id from the parent implementation row — every session
-- in an implementation belongs to that implementation's department.
