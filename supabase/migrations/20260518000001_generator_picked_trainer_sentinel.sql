-- Fix: generator anchor mode raised
--   null value in column "impl_trainer_id" of relation "tmp_intended_placements"
--     violates not-null constraint
-- when no slot fit for a session.
--
-- Root cause: the per-session "did we place it?" sentinel was
-- `v_slot_start is null`. But v_slot_start gets assigned BEFORE the
-- conflict checks fire (room overlap, trainer concurrency, weekly
-- hours), so a failed attempt leaves v_slot_start holding the
-- timestamp of the last tried slot even though no valid placement
-- happened. v_picked_trainer is only assigned on a successful pass,
-- so it's the correct "we placed this" signal.
--
-- Anchor mode just exposed this — the extra cross-impl busy state
-- tipped more sessions into "no fit", triggering the bad branch.
--
-- Body re-emitted verbatim from 20260513000010 with the single-line
-- sentinel change: line 463 now reads `if v_picked_trainer is null`.
...
begin
  ...
  loop
    ...
    for v_session_idx in 1..v_sessions_needed loop
      ...
      if v_picked_trainer is null then
