-- Fix: column reference "impl_trainer_id" is ambiguous when calling
-- find_alternative_slots from the conflict resolver. The function's
-- RETURNS TABLE declares impl_trainer_id (and impl_room_id) as OUT
-- columns, which shadows the same-named table columns inside any
-- unqualified SELECT in the function body. Postgres can't decide
-- which one to bind to and raises 42702.
--
-- Two call sites need qualification:
--   * the PTO loop reading impl_trainer_unavailability
--   * the inline subquery reading impl_sessions.impl_room_id for the
--     "skip the session's own current slot" check
--
-- This recreates the function with table aliases on every column
-- reference that collides with an OUT parameter. No behavioral change.
