-- Fix: ERROR "DROP TABLE is not allowed in a non-volatile function"
-- when calling find_alternative_slots from the conflict resolver.
--
-- The function was declared STABLE but its body creates/drops temp
-- tables and INSERTs into them — side effects only permitted in
-- VOLATILE functions. The previous ambiguous-column parse error
-- short-circuited before execution ever reached the DROP TABLE.
--
-- Drop + recreate (rather than ALTER) because the easiest cross-version
-- way to change volatility while keeping everything else identical is
-- to redefine. Body is byte-identical to 20260517000002 except for
-- removing the `stable` line.

drop function if exists public.find_alternative_slots(uuid, int);

create function public.find_alternative_slots(
  p_session_id   uuid,
...
begin
  select s.org_id, s.implementation_id, s.impl_class_id,
         s.scheduled_start, s.scheduled_end, s.impl_trainer_id, s.impl_room_id
    into v_org_id, v_impl_id, v_class_id,
         v_orig_start, v_orig_end, v_orig_trainer_id, v_orig_room_id
  ...
  for rec_pto in
    select u.impl_trainer_id as trainer_id, u.starts_at, u.ends_at
  ...
  loop
    insert into pg_temp.tmp_alt_busy_trainer
      values (rec_pto.trainer_id, rec_pto.starts_at, rec_pto.ends_at);
  end loop;
  ...
  loop
    ...
    while v_day <= v_cutoff loop
      ...
      while v_local_hr + v_duration_hours <= v_day_end_local_hr loop
        ...
        if v_slot_start = v_orig_start and rec_room.id = v_orig_room_id then
